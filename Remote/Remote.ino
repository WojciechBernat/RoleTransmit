...
#define BUFFER_SIZE        4    //Size of TX and RX buffers //zmniejszone do 4 bajtów zeby przyspiszysc transmisję i nie wysylac pustych bitow
...
void setup() {
  /* UART init */
  Serial.begin(UART_SPEED_115, SERIAL_8E1);                        //UART 8 bits with EVEN mode - że bit parzystości
  ...
  if (TxRole) { //if module is in transmitter mode
    ...
    if (TxCounter == ToRxCounter) {
      ...
      do {
        ...
        counter++;
        TxState = remote.write(TxBuffer, BUFFER_SIZE);
